<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="CartLicense.aspx.cs" Inherits="CartLicense" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="Content" Runat="Server">
   <script type="text/javascript">
    ...
    <h3 class="text-center">جواز دست فروش
    </h3>
    
     <div class="panel panel-warning">
       <div class="panel-heading">جستجو دست فروش</div>
        <div class="panel-body">
            <div class="row">
                ...
                <div class="col-md-2"><input id="txtCode" runat="server" class="form-control input-sm" placeholder="کود "/></div>
                  <div class="col-md-2">اسم دست فروش: </div>
                <div class="col-md-2"> <input id="txtName" runat="server" class="form-control input-sm auto" placeholder="اسم دست فروش  "/>
    ...
    <div class="panel panel-warning">
       <div class="panel-heading"> مشخصات عمومی دست فروش</div>
        <div class="panel-body">
            ...
            <asp:GridView runat="server" ID="gvGroup" AutoGenerateColumns="False" Width="99%" CssClass="table table-bordered table-hover"  DataSourceID="dsPaymentCategory" EmptyDataText=" هیچ دست فروش موجود نیست ">
           
       <HeaderStyle Height="30px" HorizontalAlign="Center" BackColor="#eccdb0"  />
            <Columns>
                <asp:BoundField DataField="ID" HeaderText="شماره" />
                   <asp:TemplateField>
		            <HeaderTemplate>کود دست فروش</HeaderTemplate>
		            <ItemTemplate>
                        <%#Eval("Code") %>
		            </ItemTemplate>
    ...
    <asp:UpdatePanel ID="updPayment" runat="server" ChildrenAsTriggers="true">
       ...
       <HeaderStyle Height="30px" HorizontalAlign="Center" BackColor="#eccdb0"  />
            <Columns>
                <asp:BoundField DataField="ID" HeaderText="شماره" />
                  <asp:TemplateField>
		            <HeaderTemplate>نوع سند </HeaderTemplate>
		            <ItemTemplate>
                        <%#Eval("Type") %>
		            </ItemTemplate>
                ...
                <asp:TemplateField>
		            ...
		            <ItemTemplate>
                        <asp:LinkButton ID="lnkPrint" runat="server" CssClass="glyphicon glyphicon-print" CommandArgument='<%#Eval("ID")+","+ Eval("TypeID") %>' CommandName="print" CausesValidation="false" Visible="false"></asp:LinkButton>
                      <%--  <asp:LinkButton ID="lnkEdit" runat="server" CssClass="glyphicon glyphicon-edit" CommandArgument='<%#Eval("ID") %>' CommandName="Ed" CausesValidation="false" Visible="false"></asp:LinkButton>--%>
                        <asp:LinkButton ID="lnkDelete" runat="server" CssClass="glyphicon glyphicon-remove-sign" CommandArgument='<%#Eval("ID") +","+ Eval("TypeID") %>' CommandName="Del" OnClientClick="return confirm('آیا شما مطمین هستیند؟');" CausesValidation="false" Visible="false"></asp:LinkButton>
		            </ItemTemplate>
            ...
            <asp:SqlDataSource runat="server" ID="dsPayment" ConnectionString='<%$ ConnectionStrings:Conn %>'        
                   SelectCommand="select bl.ID,dbo.ToPersianDate(bl.IssueDate) as IssueDate,dbo.ToPersianDate(bl.ExpirationDate) as ExpiryDate, s.Name_Local as Status,BusinessID, case when TypeID=1 then N'جواز' else N'شناخت کارت' end as [Type],TypeID from CartLicense bl inner join zcertificateStatus s on s.ID=bl.statusID where bl.businessID=@BusinessID">
                 <SelectParameters>
                     <asp:SessionParameter SessionField="CartIDForLicense" Name="BusinessID" />
                 </SelectParameters>
             </asp:SqlDataSource>
       ...
       <div class="panel-heading">  تنطیم جواز </div>
        <div class="panel-body">
             ...
             <div class="row">
                <div class="col-md-2">نوع سند: </div>
                <div class="col-md-4"><asp:DropDownList runat="server" ID="ddlStatus" DataSourceID="dsClass" DataTextField="Name_Local" DataValueField="ID" CssClass="form-control"></asp:DropDownList>
                                       ...
                                       SelectCommand="Select 1 as ID,N'جواز' as Name_local union select 2, N'شتاخت کارت'">
                                     </asp:SqlDataSource>
                </div>
